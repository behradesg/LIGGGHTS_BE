  ...
  Fix(lmp, narg, arg)
{
  ...
  while (iarg < narg) {
    if ((strcmp(arg[iarg],"xlo") == 0) || (strcmp(arg[iarg],"xhi") == 0) ||
      ...
      if(iarg+2 > narg) error->all(FLERR,"Illegal wall/reflect command");
      mu = atof(arg[iarg+1]);
      if (mu > 1.) error->all(FLERR,"Please choose a friction coefficient smaller than 1.");
      if (mu < 0.) error->all(FLERR,"Please choose a friction coefficient larger than 0.");
      iarg += 2;
    } else if (strcmp(arg[iarg],"beta0") == 0){
      if(iarg+2 > narg) error->all(FLERR,"Illegal wall/reflect command");
      beta0 = atof(arg[iarg+1]);
      if (beta0 > 1.) error->all(FLERR,"Please choose a tangential restitution coefficient smaller than 1.");
      if (beta0 < 0.) error->all(FLERR,"Please choose a tangential restitution coefficient larger than 0.");
      iarg += 2;
    } else error->all(FLERR,"Illegal fix wall/reflect command");
  }
  ...
  for (int m = 0; m < nwall; m++) {
    ...
    for (i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        cn = fabs(v[i][dim]);
        ct = 0;
        for (j=0; j<3; j++) ct += v[i][j]*v[i][j];
        ct = sqrt(ct - v[i][dim]*v[i][dim]);
        mu0 = 3.5*(1.0 + e)/(1.0 + beta0)*mu;
        if (side == 0) {
          if (x[i][dim] < coord) {
            x[i][dim] = coord + (coord - x[i][dim]);
            v[i][dim] = -v[i][dim] * e;
            // check for sticking or sliding collision
            if (ct < mu0*cn) {
              coeff = (1.0 + e)*(mu/mu0);
            } else {
              coeff = (1.0 + e)*(mu)*cn/(ct + 1.0e-7);
            }
            if (coeff > 1.0) coeff = 1.0; // particles should not change tangential direction
            for (j=0; j<3; j++) {
              if (j!=dim) {
                v[i][j] -= coeff*v[i][j];
              }
