  ...
  siem_flag_(false)
{
    ...
    while(iarg_ < narg && hasargs)
    {
      ...
      if (strcmp(arg[iarg_],"reference_point") == 0) {
          ...
          else if(strcmp(arg[iarg_],"SIEM") == 0) { wear_flag_ = 1; siem_flag_ = true; }
          else error->fix_error(FLERR,this,"expecting 'finnie' or 'wear' or 'off' as wear argument");
          iarg_++;
          hasargs = true;
...
{
    double E,c[3],v_rel[3],v_rel_mag,cos_gamma,sin_gamma,sin_2gamma, vrel_normal_mag, frc_normal_mag, Pt, P, impact_angle;
    double contactPoint[3]={},surfNorm[3], tmp[3], tmp2[3], contact_v[3];
    double vrel_normal[3], vrel_tangential[3], frc_normal[3], frc_tangential[3];
    ...
    if(trackWear())
    {
        ...
        if (siem_flag_)
        {
            contact_v[0] = v[0] + omega[1]*c[2] - omega[2]*c[1];
            contact_v[1] = v[1] + omega[2]*c[0] - omega[0]*c[2];
            contact_v[2] = v[2] + omega[0]*c[1] - omega[1]*c[0];

            // calculate relative velocity
            vectorSubtract3D(contact_v,v_wall,v_rel);
            ...
            vrel_normal_mag = vectorDot3D(v_rel, surfNorm);
            vrel_normal[0] = surfNorm[0] * vrel_normal_mag;
            vrel_normal[1] = surfNorm[1] * vrel_normal_mag;
            vrel_normal[2] = surfNorm[2] * vrel_normal_mag;

            //calculate tangential velocity vector
            vectorSubtract3D(v_rel, vrel_normal, vrel_tangential);
            //printf("relative velocity is: (%e, %e, %e) \n", v_rel[0], v_rel[1], v_rel[2]);
            //printf("relative tangential velocity is: (%e, %e, %e) \n", vrel_tangential[0], vrel_tangential[1], vrel_tangential[2]);


            //calculate the tangential force on the surface
            vectorNegate3D(frc);
            vectorCross3D(frc, surfNorm, frc_tangential);

            //calculate the relative normal velocity
            frc_normal_mag = vectorDot3D(frc, surfNorm);
            frc_normal[0] = surfNorm[0] * frc_normal_mag;
            frc_normal[1] = surfNorm[1] * frc_normal_mag;
            frc_normal[2] = surfNorm[2] * frc_normal_mag;
            //printf("normal force is: (%e, %e, %e) \n", frc_normal[0], frc_normal[1], frc_normal[2]);

            //calculate tangential velocity vector
            vectorSubtract3D(frc, frc_normal, frc_tangential);
            //printf( "tangential force is: (%e, %e, %e) \n", frc_tangential[0], frc_tangential[1], frc_tangential[2]);
            Pt = vectorDot3D(frc_tangential, vrel_tangential);
            P = vectorDot3D(frc_tangential, v_rel);
            if (Pt < 0 && P < 0) {
                E = -vectorDot3D(frc_tangential, vrel_tangential)*update->dt;
                //printf("Energy is: %e\n",  E);
            } else {
                E = 0.0;
            }

            wear_step(iTri) += E/(4.0*k_finnie_[atomTypeWall()-1][atom->type[ip]-1]);
            //printf("Energy itri is: %d %e\n", iTri, wear_step(iTri));
