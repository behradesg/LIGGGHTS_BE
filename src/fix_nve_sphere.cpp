  ...
  fix_cfd_coupling_force_implicit_(0)
{
  ...
  while (iarg < narg) {
    ...
    else if(strcmp(arg[iarg],"omega_integration") == 0)
    {
        if(narg < iarg+2)
            error->fix_error(FLERR,this,"not enough arguments for 'omega_integration'");
        iarg++;
        if(strcmp(arg[iarg],"yes") == 0)
            omegaIntegration_ = true;
        else if(strcmp(arg[iarg],"no") == 0)
            omegaIntegration_ = false;
        else
            error->fix_error(FLERR,this,"expecting 'yes' or 'no' after 'omega_integration'");
        iarg++;
    }
    else error->all(FLERR,"Illegal fix nve/sphere command");
  }
